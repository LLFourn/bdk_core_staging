...
fn test_chain_graph_relevant_changeset() {
    // TODO: right now this test is panicking with:
    // thread 'test_chain_graph_relevant_changeset' panicked at
    // 'should have full tx at this point',
    // /src/chain_graph.rs:366:17
    // This happens because determine_relevant_changeset inserts
    // the non-relevant txs in the SparseChain, but not in the TxGraph.
    let tx_a = Transaction {
        version: 0x01,
        lock_time: PackedLockTime(0),
        input: vec![],
