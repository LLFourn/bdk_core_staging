...
impl ElectrumClient {
    ...
    pub fn wallet_txid_scan<K: Ord + Clone>(
        ...
        let mut keychain_scan = KeychainScan::<K, TxHeight>::default();
        ...
        for block in block_ids {
            keychain_scan.update.insert_checkpoint(block)?;
        }
        ...
        for (keychain, mut scripts) in scripts.into_iter() {
            ...
            // Fetch and add all transactions into `KeyChainScan`.
            let transactions =
                self.batch_transaction_get(script_history_txid.iter().map(|(txid, _)| txid))?;

            for (tx, index) in transactions
                .into_iter()
                .zip(script_history_txid.into_iter().map(|(_, index)| index))
            {
                keychain_scan.update.insert_tx(tx, Some(index))?;
            }
