...
impl<I: Clone + Ord> SpkTracker<I> {
    ...
    fn add_tx(
        ...
        for (i, out) in tx.output.iter().enumerate() {
            if let Some(index) = self.index_of_spk(&out.script_pubkey).cloned() {
    ...
    pub fn iter_unused(&self) -> impl Iterator<Item = (I, &Script)> {
        self.unused
            .iter()
            .map(|index| (index.clone(), self.spk_at_index(index).expect("must exist")))
    }
    ...
    pub fn create_psbt(
        ...
        mut lookup_descriptor: impl FnMut(I) -> Option<Descriptor<DefiniteDescriptorKey>>,
...
impl<K: Ord + Clone> SpkTracker<(K, u32)> {
    ...
    pub fn derive_spks(
        ...
        for index in next_to_derive..=end {
            let spk = descriptor
                ...
                .script_pubkey();
            self.add_spk((keychain.clone(), index as u32), spk);
        }
    ...
    pub fn derive_new(
        ...
        self.add_spk(index.clone(), new_spk);
        let new_spk = self.script_pubkeys.get(&index).expect("we just added it");
    ...
    pub fn derive_next_unused(
        ...
        let need_new = self.iter_unused().next().is_none();
        // this rather strange branch is needed because of some lifetime issues
        if need_new {
            self.derive_new(keychain, descriptor)
        } else {
            self.iter_unused()
...
mod test {
    ...
    fn no_checkpoint_and_then_confirm() {
        ...
        tracker.derive_spks((), &update_gen.descriptor, 0);
    ...
    fn two_checkpoints_then_merege() {
        ...
        let mut update_gen = UpdateGen::new();
        let mut tracker = SpkTracker::default();
        tracker.derive_spks((), &update_gen.descriptor, 3);
